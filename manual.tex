...
\chapter{REFERÊNCIA USANDO O abn\TeX}\label{chap:referencia}
	...
	\emph{Nota:} antes do advento do \texttt{xindy}, era comum (e ainda o é) a comunidade \index{Latex@\LaTeX}\LaTeX~utilizar a ferramenta \texttt{makeindex}. Pelo fato do \texttt{makeindex} não suportar idiomas que não sejam o inglês, foi decidido importar o \texttt{xindy} neste projeto. No entanto, se você quiser dicas avançadas para a criação de índices, recomenda-se a leitura da documentação do \texttt{makeindex} \url(http://www.ctan.org/pkg/makeindex), uma vez que ambos os programas seguem o mesmo padrão de codificação.
