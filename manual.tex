...
\section{Pacotes necessários}\label{sec:pacotes}
    ...
    \begin{enumerate}
        ...
        \item\index{glossaries}\texttt{glossaries}: permite a criação de listas de símbolos e abreviaturas;
...
\chapter{CITAÇÕES USANDO O abn\TeX}\label{chap:referencia}
	...
	Para a criação de índices remissivos, foi importado o pacote \texttt{imakeidx}. Este pacote utiliza o programa \index{makeindex}\texttt{makeindex} na criação de arquivos auxiliares que indexam as palavras a serem colocadas no índice. Normalmente, o \index{makeindex}\texttt{makeindex} vem junto com a instalação do \LaTeX.
	
	\emph{Nota:} pelo fato do \index{makeindex}\texttt{makeindex} não processar palavras que não sejam do idioma inglês muito bem, foi criado um sucessor espiritual para ele, o \texttt{xindy}. No entanto, o \texttt{xindy} não possui versões para Windows e Mac OS, então foi optado utilizar o \index{makeindex}\texttt{makeindex} neste projeto. Para aprender dicas avançadas na criação de índices mais complexos, recomenda-se a leitura da documentação do \index{makeindex}\texttt{makeindex} \url(http://www.ctan.org/pkg/makeindex).
	...
	Para a criação das listas de símbolos e abreviaturas, foi utilizado o pacote \index{glossaries}\texttt{glossaries}, responsável por indexar termos de diferentes categorias e gerar listas destes termos. Ao contrário do índice, que indexa as palavras no decorrer do texto, o pacote \index{glossaries}\texttt{glossaries} exige que os termos sejam declarados antes de serem referenciados durante o texto. Uma boa prática para organizar tais termos consiste em declará-los ao início do documento, ou em um documento separado, o qual pode ser chamado utilizando os comandos \latexinline+\input+ ou \latexinline+\include+. Estas opções ficam a cargo do leitor. As próximas duas seções ensinarão os comandos básicos para indexação de símbolos e abreviaturas.
	
	\emph{Nota:} assim como descrito no capítulo \ref{chap:indice}, o pacote \index{glossaries}\texttt{glossaries} depende das ferramentas \index{makeindex}\texttt{makeindex}.
	...
	O pacote \index{glossaries}\texttt{glossaries} disponibiliza os seguintes comandos para chamar os itens indexados durante o texto:
	...
	Repare que, no caso das siglas, quando estas são usadas pela primeira vez, são impressas a definição seguida da sigla entre parênteses. Nas demais vezes, a sigla aparecerá sozinha. É importante ressaltar que o pacote \index{glossaries}\texttt{glossaries} adiciona às listas somente os termos que foram utilizados durante o texto. Para que todos os termos declarados apareçam, basta usar o comando \latexinline+\glsaddall+ no corpo do texto.
	...
	Para a utilização de todos os recursos que a classe \texttt{fei.cls} disponibiliza, é necessário compilar o projeto utilizando diversas ferramentas diferentes em ordem específica:
...
pdflatex manual.tex
	...
	\begin{enumerate}
	\item \texttt{alg}: \emph{log} do \index{makeindex}\texttt{makeindex};
	...
	\item \texttt{idx}: arquivo com os termos a serem adicionados no índice pelo \index{makeindex}MakeIndex;	
